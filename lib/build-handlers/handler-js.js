...
class JSHandler {
    ...
    pipePageJS(blob, page) {
        ...
        let absoluteTS = [];
        if (this.$config.page_bundles[page] && this.$config.page_bundles[page].ts) {
            absoluteTS = FileUtils.absolutePathList(this.$config.source.ts, this.$config.page_bundles[page].ts);
        }
